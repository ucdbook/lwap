require(['three'], function(THREE) {
    ...
    var threeLoginBg = disk();
    var bg = new threeLoginBg($('.canvas-bg'));
    bg.init();
    adapt();
})
...
function adapt(){
    var body = $('body');
    $('.loginIndexPage').height(body.height());
}

function disk(){
    var ThreeLoginBackground = function ($canvas) {
        var I = this;
        ...
        function drawCircle1() {
            loadTexture('/scms/login/images/pic1.png', function ( map ) {//创建一个纹理映射，将其添加到场景中
        ...
        function drawCircle2() {
            loadTexture('/scms/login/images/pic2.png', function ( map ) {//创建一个纹理映射，将其添加到场景中
                var material = new THREE.MeshBasicMaterial({ //创建一个基础材质，传入纹理映射参数
                    map: map,
                    transparent: true,
                    opacity: 1
        ...
        function drawCircle3() {
            loadTexture('/scms/login/images/pic3.png', function ( map ) {//创建一个纹理映射，将其添加到场景中
                var material = new THREE.MeshBasicMaterial({ //创建一个基础材质，传入纹理映射参数
                    map: map,
                    transparent: true
                });
        ...
        function drawCircle4() {
            loadTexture('/scms/login/images/pic4.png', function ( map ) {//创建一个纹理映射，将其添加到场景中

                var material = new THREE.MeshBasicMaterial({ //创建一个基础材质，传入纹理映射参数
                    map: map,
                    transparent: true
                });
                var geometry = new THREE.PlaneGeometry(0.38, 0.38, 2, 2);
                
                I.cubeCircle4 = new THREE.Mesh(geometry, material);//将几何形状和材质整合到一个网格中
                I.cubeCircle4.position.z = -5.1;
                I.cubeCircle4.position.y = -0.75;
        ...
        function getWordPosition(pos){
            ...
            pm.makeRotationFromQuaternion(new THREE.Quaternion(0.50, 0, 0, 0));
        ...
        function moveWords(){
            ...
            for(var i=0;i<count;i++){
                ...
                var pos = {
                    ...
                    y: I.wordRadius * Math.cos(range) - 0.4,//文字y坐标
                    z: -6
                }

                var p = getWordPosition(pos);
                p.z = p.z * Math.cos(range) / 9 - 1.5 ; //修改z坐标
        ...
        function drawWord(word){
            loadTexture('/scms/login/images/' + word.pic, function ( map ) {
                var material = new THREE.MeshBasicMaterial({ //创建一个基础材质，传入纹理映射参数
                    map: map,
                    transparent: true//透明
                });
                var geometry = new THREE.PlaneGeometry(0.12, 0.052, 2, 2);
        ...
        function drawLines(){
            ...
            createDrawLine({
                    imgUrl: '/scms/login/images/light1.png',
                    pos:{x: -1, y: 1.6 }
                }, function(cube){
                I.cubeLine11 = cube;
            ...
            createDrawLine2({
                    imgUrl: '/scms/login/images/light2.png',
                    pos:{x: 0.8,    y: 1.6 }
                }, function(cube){
                I.cubeLine31 = cube;
                group.add(cube);//将网格添加到场景中
